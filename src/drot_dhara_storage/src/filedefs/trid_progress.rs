//! TrID build progress: console, audit, and operation-plan updates.
...
const AUDIT_TARGET: &str = "drot::audit";
...
fn progress_settings() -> ProgressSettings {
    PROGRESS_SETTINGS
        .get()
        .copied()
...
fn interactive_mode_enabled() -> bool {
    // Prefer kernel settings when available (set during logging init).
    drot_kernel::logging::interactive_mode_enabled()
}
...
fn console_enabled() -> bool {
    ...
    progress_settings().console_enabled()
        || (!interactive_mode_enabled() && std::io::stderr().is_terminal())
}
...
thread_local! {
    ...
    static TRID_PLAN_READY: Cell<bool> = const { Cell::new(false) };
}
...
pub fn reset_build_progress_logging() {
    ...
    TRID_PLAN_READY.with(|ready| ready.set(false));
}
...
pub fn dispatch_trid_progress(update: &TridBuildProgress) {
    with_progress_lock(|| {
        ...
        apply_trid_progress(update);
        log_audit_progress(update);
    });
}

/// Maps TrID build progress into the kernel operation plan via public APIs.
pub fn apply_trid_progress(update: &TridBuildProgress) {
    match update.stage {
        TridBuildStage::LoadSource => {
            if !has_committed_progress_plan() && !TRID_PLAN_READY.with(|ready| ready.get()) {
                begin_analyzing("Analyzing TrID source…");
            }
        }
        TridBuildStage::ExtractArchive => {
            ensure_trid_plan();
            let total = update.total.unwrap_or(1).max(1) as u64;
            set_step_total("extract", total);
            tick_step("extract", update.current as u64);
            set_step_message("extract", "Extracting archive…");
        }
        TridBuildStage::ParseDefinitions => {
            ensure_trid_plan();
            if let Some(total) = update.total.filter(|total| *total > 0) {
                set_step_total("parse", total as u64);
                set_step_total("reduce", total as u64);
            }
            tick_step("parse", update.current as u64);
            if update.message.starts_with("Reading definition files") {
                set_step_message("parse", "Reading definition files…");
            } else {
        ...
        TridBuildStage::ReduceDefinitions => {
            ...
            tick_step("reduce", update.current as u64);
            let total = update.total.unwrap_or(0);
            set_step_message(
                "reduce",
        ...
        TridBuildStage::FinalizePackage => {
            ensure_trid_plan();
            let total = update.total.unwrap_or(1).max(1) as u64;
            set_step_total("finalize", total);
            tick_step("finalize", update.current as u64);
            set_step_message("finalize", "Finalizing package…");
        }
    }
}
...
fn ensure_trid_plan() {
    ...
    // Clear any placeholder single-shot plan, then commit the four TrID steps.
    begin_analyzing("Analyzing TrID source…");
    plan_step("extract", "Extracting archive", 1);
    plan_step("parse", "Parsing definitions", 1);
    plan_step("reduce", "Reducing definitions", 1);
    plan_step("finalize", "Finalizing package", 1);
    set_step_total("extract", 1);
    set_step_total("finalize", 1);
    commit_plan();
    TRID_PLAN_READY.with(|ready| ready.set(true));
}
...
fn maybe_log_reduce_trace(update: &TridBuildProgress) {
    let trace =
        progress_settings().trace || tracing::enabled!(target: AUDIT_TARGET, tracing::Level::DEBUG);
    if !trace {
        return;
    }
...
pub fn phase_activity_label(stage: TridBuildStage) -> &'static str {
    match stage {
