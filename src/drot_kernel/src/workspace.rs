...
/// Metadata recorded after writing a definitions package (product-supplied).
#[derive(Debug, Clone, PartialEq, Eq)]
...
fn analyze_defs_package(defs_path: &Path) -> WorkspaceSnapshot {
    if let Some(hooks) = crate::product::product_hooks() {
        return hooks.analyze_defs_package(defs_path);
    }
...
pub fn record_package_written(path: &Path, package: &PackageMeta) {
    let Some(mutex) = WORKSPACE.get() else {
        return;
    };
    ...
    state.snapshot.definition_count = Some(package.definition_count);
}
...
mod tests {
    ...
    fn sample_meta(version: &str, revision: u16) -> PackageMeta {
        PackageMeta {
    ...
    #[test]
    fn present_file_without_hooks_is_present_without_meta() {
        let temp = tempdir().unwrap();
        let defs_path = default_defs_package_path(temp.path());
        fs::create_dir_all(defs_path.parent().unwrap()).unwrap();
        fs::write(&defs_path, b"placeholder").unwrap();

        let snapshot = analyze_defs_package(&defs_path);
        assert_eq!(snapshot.defs_status, DefsPackageStatus::Present);
        assert!(snapshot.package_revision.is_none());
        assert_eq!(snapshot.next_package_revision("0.6.0").unwrap(), 1);
    }
    ...
    fn record_package_written_updates_cached_revision() {
        ...
        // Without hooks, presence alone does not encode revision → next is 1.
        assert_eq!(
            next_package_revision(&context, "0.6.0").expect("revision"),
            1
        );

        record_package_written(&defs_path, &sample_meta("0.6.0", 3));
        assert_eq!(
            next_package_revision(&context, "0.6.0").expect("revision"),
            4
        );
