...
const DEFAULT_EMBEDDED_DEFS_DIR_RELATIVE: &str = "src/core/dhara_storage_dal/resources";
const DEFAULT_EMBEDDED_DEFS_RELATIVE: &str = "src/core/dhara_storage_dal/resources/filedefs.dat";
...
pub fn embedded_defs_dir_relative() -> &'static str {
    crate::product::product_hooks()
        .map(|hooks| hooks.embedded_defs_dir_relative())
        .unwrap_or(DEFAULT_EMBEDDED_DEFS_DIR_RELATIVE)
}

/// Relative path from the repository root to the embedded runtime defs package.
pub fn embedded_defs_relative() -> &'static str {
    crate::product::product_hooks()
        .map(|hooks| hooks.embedded_defs_relative())
        .unwrap_or(DEFAULT_EMBEDDED_DEFS_RELATIVE)
}

/// Relative path from the repository root to the embedded runtime defs package.
pub fn runtime_defs_relative() -> &'static str {
    embedded_defs_relative()
}
