...
pub fn log_session_end(exit_code: i32, module_id: Option<&str>, error: Option<&str>) {
    ...
    match (exit_code, module_id, error) {
        (0, Some(module), None) => info!(
            target: AUDIT_TARGET,
            "drot exiting 0 at {timestamp} — completed {module}"
        ),
        (code, Some(module), Some(err)) => info!(
            target: AUDIT_TARGET,
            "drot exiting {code} at {timestamp} — {module} failed: {err}"
        ),
        (code, _, Some(err)) => info!(
            target: AUDIT_TARGET,
            "drot exiting {code} at {timestamp} — {err}"
        ),
        (code, Some(module), None) => info!(
            target: AUDIT_TARGET,
            "drot exiting {code} at {timestamp} — {module}"
        ),
        (code, None, None) => info!(
            target: AUDIT_TARGET,
            "drot exiting {code} at {timestamp}"
        ),
    }
...
mod tests {
    ...
    fn log_file_name_uses_daily_stem() {
        ...
        assert_eq!(log_file_name_for(date), "2026-04-10_drot.log");
    }
