...
`drot` is the operator CLI for the Dhara Storage workspace.
...
tooling/drot/
├── crates/
│   ├── drot_kernel/   # paths, config, logging, defs I/O, operation_progress
│   │   └── data/            # compile-time reference catalogs (embedded via include_str!)
│   ├── drot_ops/      # quality, verify, release, native merge
│   ├── drot_cli/      # registry, commands, forms, runner, interactive state
│   ├── drot_tui/      # ratatui screens, widgets, event loop
│   └── drot/          # binary entry (CLI + TUI boot)
...
├── logs/                # audit logs ({date}_drot*.log)
...
With the dist binary (`target/dist/drot`), `exe_path` is `target/dist/`. `cargo run` uses `target/debug/` instead. Repository-scoped outputs (embedded `filedefs.dat` sync target) stay under the workspace via `-r` — see [logging reference][logging]. TrID input archives ship beside the binary in `{exe_path}/package/`.
...
**Tool versioning:** bump `[tool].version` in [dhara.config.toml][dhara-config] and `[workspace.package].version` in [tooling/drot/Cargo.toml](Cargo.toml) together for tool-only changes (member crates inherit via `version.workspace = true`). After `version bump`, the next run offers to sync root `Cargo.toml` and the NuGet csproj from config. CI uses `--yes` to apply drift without prompting.

**Dist vs dev:** production-shaped binary lives at `target/dist/drot` (`[profile.dist]`). [`ensure-dhara-tool-dist`][ensure-dist-ps1] rebuilds only when the binary is missing or `--version` ≠ manifest. Use `cargo run -p drot` for day-to-day tool edits without invalidating dist.

```powershell
./tooling/scripts/ensure-dhara-tool-dist.ps1
./tooling/scripts/verify-local.ps1
./target/dist/drot -r . --yes config show
./target/dist/drot -r . --yes package stage-native --msvc-env
./target/dist/drot -r . --yes native merge --output target/dist/artifacts/native-stage --input ...
./target/dist/drot -r . --yes verify package
./target/dist/drot -r . --yes release run --dry-run
```
...
```powershell
cargo test -p drot
cargo clippy -p drot --all-targets -- -D warnings
```

CI builds `profile.dist` on Windows (`stage-native`) and Linux (`package pack`, `verify package`; cache keyed by source hash). Rust/dotnet tests run on `platform (linux)` only; other platform jobs stage natives. PR CI does not run `cargo test -p drot` — validate locally.

**VS Code:** tasks under `dhara-tool:` — `ensure dist`, `watch dev` (`cargo watch`, dev profile), `quality run (dist)`. Launch **Debug drot (dev)** for `cargo run`; **Run drot (dist)** ensures dist first. Requires [CodeLLDB][codelldb]; `cargo-watch` for the watch task.
...
Audit logs land in `{tool_root}/logs/{date}_drot[_N].log` (e.g. `target/dist/logs/` after `ensure-dhara-tool-dist`).
