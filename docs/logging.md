...
```
drot 0.10.0 started — mode=direct, workers=4, extension=dhara_storage
```
...
```
INFO  drot 0.10.0 started — mode=interactive, workers=4, extension=dhara_storage
...
```
INFO  drot 0.10.0 started — mode=direct, workers=4, extension=dhara_storage
...
- TrID archive extract reports entry-level progress in the TUI (`Extracting archive (k/N)`) when [`sevenz-rust`](../crates/drot_dhara_storage/Cargo.toml) succeeds; `tar` fallback stays indeterminate.
...
[command-run]: ../crates/drot_kernel/src/logging/operation.rs
