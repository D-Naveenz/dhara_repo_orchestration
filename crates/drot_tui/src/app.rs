...
fn handle_form_edit_key(app: &mut DharaTui, key: KeyEvent) -> Result<()> {
    ...
    match key.code {
        ...
        KeyCode::Char(' ')
            if field.is_some_and(|f| matches!(f.kind, drot_kernel::FieldKind::Boolean)) =>
        {
        ...
        KeyCode::Char(_) if get_char(&key).is_some() => {
            if field.is_some_and(|f| {
                matches!(
                    f.kind,
                    drot_kernel::FieldKind::Text
                        | drot_kernel::FieldKind::Path
                        | drot_kernel::FieldKind::BrowsablePath { .. }
                )
