Build input for large operator files that ship beside the `drot` binary.
...
- Launching `drot` without a subcommand opens the interactive GUI when a
...
[`crates/drot_kernel/data/`](../crates/drot_kernel/data/) and are
